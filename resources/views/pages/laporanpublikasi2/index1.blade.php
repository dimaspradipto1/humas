        ...
        <table class="table table-hover" id="crudTable">
          <thead>
            <tr>
              ...
              <th class="text-uppercase">nama fakultas</th>
              <th class="text-uppercase">nama kegiatan</th>
              <th class="text-uppercase">link dokumen</th>
            </tr>
          </thead>
          <tbody>
            {{-- @if (pdi->isNotEmpty())
              @foreach ($pdi as $item)
                <tr>
                  ...
                  <td style="width: 30%">
                    {{ $item->judul }}
                    <br>
                    <p style="color: #3FA2F6">
                      @if ($item->user->nama_lengkap)
                      {{ $item->user->nama_lengkap }}, {{ $item->user->nidn }}
                      @endif
                  ...
                  <td >
                    <a href="{{ Storage::url($item->file) }}" class="text-success" target="_blank"
                      style="text-decoration: none">Download Dokumen Jurnal</a>
                    <br>
                  </td>
                  <td style="width: 10%"><a href="{{ $item->url }}" target="_blank" style="text-decoration: none; color: #0F67B1">view link jurnal</a></td>
                  <td style="width: 10%">{{ $item->tahun}}</td>
                  <td class="text-success">{{ $item->status }}</td>
                </tr>
              @endforeach
            @else
              <tr>
                <td colspan="5" class="text-center">Tidak ada data pengabdian dosen.</td>
              </tr>
            @endif --}}
          </tbody>
        </table>
