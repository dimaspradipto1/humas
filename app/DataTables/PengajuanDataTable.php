        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('status', function($pengajuan){
                if($pengajuan->status == 'pending'){
                    return '<span class="badge bg-warning px-2 rounded-pill">Pending</span>';
                }elseif($pengajuan->status == 'disetujui'){
                    return '<span class="badge bg-success text-white px-2 rounded-pill">disetujui</span>';
                }elseif($pengajuan->status == 'ditolak'){
                    return '<span class="badge bg-danger text-white px-2 rounded-pill">ditolak</span>';
                }
            })
            ->addColumn('action', function($pengajuan) use ($user){
                if($user->is_admin){
                    return '
                    <a href="'.route('pengajuan.show', $pengajuan->id).'" class="btn btn-sm btn-primary text-white px-3 fa-solid fa-eye" ><i class="fa-solid fa-eye"></i> View</a>
                    <a href="'.route('pengajuan.edit', $pengajuan->id).'" class="btn btn-sm btn-warning text-white px-3 fa-solid fa-pen-to-square" ><i class="fa-solid fa-pen-to-square"></i> Edit</a>
